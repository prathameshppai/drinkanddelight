...
public class DisplayRawMaterialOrdersBetweenDatesServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public DisplayRawMaterialOrdersBetweenDatesServlet() {
		super();
...
//        response.getWriter().append("Served at: ").append(request.getContextPath());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			rmoList = RawMaterialServiceObject.displayRawmaterialOrdersbetweenDetails(date1, date2);
		...
		if (errormessage.isEmpty()) {
			...
			String upperhtml = "<!DOCTYPE html>\r\n" + "<html>\r\n" + "<head>\r\n" + "<meta charset=\"ISO-8859-1\">\r\n"
					+ "<title>DisplayRawMaterial Orders Between Dates</title>\r\n"
			...
			for (RawMaterialOrder po : rmoList)

			{
				upperhtml += "<div class=\"table-responsive\">" + "<table class=\"table table-striped\">" + "<tr> <td>"
						...
						+ "</td>&nbsp;<td>" + po.getRmId() + "</td><td>" + po.getSupplierId() + "</td><td>"
						+ po.getQuantityValue() + "</td><td>" + po.getQuantityUnit() + "</td><td>" + po.getDateOfOrder()
						+ "</td><td>" + po.getDeliveryStatus() + "</td><td>" + po.getPricePerUnit() + "</td><td>"
			...
			RequestDispatcher rd = req.getRequestDispatcher("/DisplayRawMaterialOrdersBetweenDates.html");
			rd.include(req, res);	
		}
	}
}